package at.ski.noophaestus.config
...
object NoophaestusClientConfig {
    @JvmStatic
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            NoophaestusServerConfig.holder.config = NoophaestusServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            NoophaestusServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Noophaestus.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use NoophaestusServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = NoophaestusServerConfig.ServerConfig()
    }
